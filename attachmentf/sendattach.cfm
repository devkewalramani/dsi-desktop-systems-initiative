...
<cfquery name="checkattach" datasource="DSI">
select custid
from customer
...
<cfquery name="addcust" datasource="DSI">
INSERT INTO customer (custid,lname,fname,office,func,officer,bal,logon,ph,mtype,status,condata,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,comment) VALUES ('#form.id#','#form.lname#','#form.fname#','#form.office#','#form.func#','#form.officer#','#form.bal#','#form.logon#','#form.ph#','#form.mtype#','#form.status#','#form.condata#','#form.s1#','#form.s2#','#form.s3#','#form.s4#','#form.s5#','#form.s6#','#form.s7#','#form.s8#','#form.s9#','#form.s10#','#form.s11#','#form.s12#','#form.s13#','#form.s14#','#form.s15#','#form.s16#','#form.s17#','#form.s18#','#form.s19#','#form.s20#','#form.comment#')
</cfquery>

<cfquery name="updatemaster" datasource="DSI">
UPDATE master SET attachstatus='pending' WHERE ID=#form.id#
</cfquery>

<cfmail from="DSI WEBSITE" to="dev.kewalramani@ny.frb.org" subject="#form.fname#, #form.lname# has filled out a Attachment F">
...
<p align="center" class="burgundy"><strong>Thank you<br>
  Your Attachment F has been Submitted to your BAL.</strong></p>
...
<table width="220"  border="0" align="center" cellpadding="5" cellspacing="5">
  <tr align="center" valign="middle" class="burgundy">
	...
	<form>
    <td width="50%">
    <input name="Submit" type="submit" class="burgundy" value="No Thank You" onClick="window.close(true)" >
    </td></form>
  </tr>
</table>
