...
--->
		  ...
		  <cfquery name="totct" datasource="DSI">
...
<!--- If ShowDays is TRUE then we can display a number in this sq to represent a day --->
  ...
  from master
  WHERE actualdate <> null AND actualdate=#parsedatetime(miday2)# 
  group by actualdate
  </cfquery>
...
<!--- dont forget to renable output incase you want to add more below the calendar HAVE FUN!---><cfsetting enablecfoutputonly="no" showdebugoutput="no"></td>
