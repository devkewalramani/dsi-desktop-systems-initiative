
<html><!-- InstanceBegin template="/Templates/dsi.dwt" codeOutsideHTMLIsLocked="false" -->
<head>
<!-- InstanceBeginEditable name="doctitle" -->
<title></title>
<cfquery name="detail" datasource="DSI">
SELECT *
FROM master
WHERE ID = #URL.recordID# 
</cfquery>


<!-- InstanceEndEditable -->
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<!-- InstanceBeginEditable name="head" -->
<style type="text/css">
<!--
.style4 {font-size: 12px}
-->
</style>
<style type="text/css">
<!--
.style6 {
	color: #6F201C;
	font-size: 16px;
	font-weight: bold;
	font-style: italic;
}
-->
</style>
<style type="text/css">
<!--
.style2 {font-weight: bold}
-->
</style>
<style type="text/css">
<!--
.style7 {font-size: 10px}
-->
</style>
<style type="text/css">
<!--
.style8 {font-size: 9px}
-->
</style>
<style type="text/css">
<!--
.style9 {color: #6F201C}
-->
</style>
<!-- InstanceEndEditable -->
<link href="../scripts/font.css" rel="stylesheet" type="text/css">

</head>

<body link="#660000" vlink="#660000">
<div id="info" style="position:absolute; left:181px; top:331px; width:224px; height:197px; z-index:1; visibility: hidden;"></div>
<div id="info2" style="position:absolute; left:181px; top:134px; width:224px; height:197px; z-index:1; visibility: hidden;"> 
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr> 
      <td align="center" bgcolor="#cccccc"> 
        <table width="100%" border="0" cellspacing="1" cellpadding="0">
          <tr> 
            <td valign="top" bgcolor="#eeeeee"><div align="center" class="burgundy"><strong>.:: 
                info box ::.</strong></div></td>
          </tr>
          <tr> 
            <td valign="top" bgcolor="#FFFFFF"> 
              <table width="100%" border="0" cellspacing="0" cellpadding="8">
                <tr> 
                  <td valign="top" class="text1">Entering a user id or last name 
                    will allow you to view all tickets. Select the &quot;Open 
                    Tickets&quot; option to view only open tickets. <p>Enter Ticket 
                      No, User ID, or Lastname.</p></td>
                </tr>
              </table>
            </td>
          </tr>
          <tr> 
            <td height="20" bgcolor="#eeeeee"> <div align="center" class="burgundy"><strong>.:<a href="javascript:;" onClick="MM_showHideLayers('info2','','hide')"><img src="../images/close1.jpg" width="100" height="15" border="0"></a>:.</strong></div></td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</div>
<table width="75%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="center" bgcolor="#cccccc"> 
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr> 
          <td valign="top" bgcolor="#FFFFFF"> 
            <table width="100%" border="0" cellspacing="0" cellpadding="4">
              <tr>
                <td width="61%" align="left" valign="top"><img src="../images/nuLogo.jpg" alt="Information Technology Logo" width="313" height="101" border="0"></td>
                <td width="39%" colspan="2" align="right">&nbsp; </td>
              </tr>
            </table></td>
        </tr>
        <tr> 
          <td valign="top" bgcolor="#eeeeee"> <table width="100%" border="0" cellspacing="0" cellpadding="2">
              <tr valign="top"> 
                <td width="1%">&nbsp;</td>
                <td width="48%" class="topmenu"> <script language="JavaScript">

    // -- made by A1javascripts.com, please keep these credits when using this script
    days = new Array(7)
    days[1] = "Sunday";
    days[2] = "Monday";
    days[3] = "Tuesday"; 
    days[4] = "Wednesday";
    days[5] = "Thursday";
    days[6] = "Friday";
    days[7] = "Saturday";
    months = new Array(12)
    months[1] = "January";
    months[2] = "February";
    months[3] = "March";
    months[4] = "April";
    months[5] = "May";
    months[6] = "June";
    months[7] = "July";
    months[8] = "August";
    months[9] = "September";
    months[10] = "October"; 
    months[11] = "November";
    months[12] = "December";
    today = new Date(); day = days[today.getDay() + 1]
    month = months[today.getMonth() + 1]
    date = today.getDate()
    year=today.getYear(); 
if (year < 2000)
year = year + 1900;
    document.write (day +
    ", " + month + " " + date + ", " + year)
    // -- end hiding 
    </script> </td>
                <td width="51%" class="topmenu"><div align="right"><a href="http://www.it.ny.frb.org/wcsd-dsi/">DSI Home </a> 
                    | <a href="http://www.it.ny.frb.org/">IT Website </a>  | <a href="http://atthebank.ny.frb.org/@theBank/currentweek/cover.htm">@TheBank</a> 
                  </div></td>
              </tr>
            </table></td>
        </tr>
        <tr> 
          <td valign="top" bgcolor="#FFFFFF">
<table width="100%" border="0" cellspacing="0" cellpadding="8">
              <tr>
                <td valign="top"><!-- InstanceBeginEditable name="mainbody" -->
				
<table width="100%" border="0" cellpadding="0" cellspacing="1" >
  <tr>
    <td width="709" height="324" valign="top">
      <table width="100%" height="324" border="0" cellpadding="0" cellspacing="1" >
  <tr>
    <td width="603" height="303" align="left" valign="top">
	<!--display information-->
	<div align="center" class="style6">Admin Re-Schedule Mode	</div>	
	<cfoutput query="detail">
	<cfform name="form" method="post"  action="updateschedulescript.cfm">
	<input type="hidden" name="ID" value="#ID#"> 
<input type="hidden" name="midate" value="#midate#">
<input type="hidden" name="lname" value="#lname#">
	     <input type="hidden" name="fname" value="#fname#">
<input type="hidden" name="funcname" value="#func#">
<input type="hidden" name="oldodate" value="#odate#">
<input type="hidden" name="oldtdate" value="#tdate#">
<input type="hidden" name="oldactualdate" value="#actualdate#">
<input type="hidden" name="oldstime" value="#stime#">
<input name="name" type="text" value="#lname#, #fname#" readonly="yes" class="burgundy"  style="background-color:##eeeeee ">
<br>
<div style="border:1px black solid; width:700px; height:85px; overflow-y:auto; overflow-x:auto;">
<table width="700" border="0">
  <tr align="left" valign="bottom">
    <th scope="col"><span class="style4">Location</span></th>
    <th scope="col"><span class="style4 style4">Floor</span></th>
    <th scope="col"><span class="style4 style4">Cube/Office</span></th>
    <th scope="col">Phone</th>
    <th scope="col"><span class="style4 style4">Work Hours</span></th>
    <th scope="col"><span class="style4 style4">Black Berry</span></th>
    <th scope="col"><span class="style4 style4">Machine Type</span></th>
    <th scope="col"><span class="style4 style4">Disposition</span></th>
    <th scope="col"><span class="style4 style4">Machine Description</span></th>
    <th scope="col"><span class="style4 style4">Serial</span></th>
    <th scope="col"><span class="style4 style4">Replacement Model</span></th>
    <th scope="col"><span class="style4 style4">Status</span></th>
    <th scope="col"><p align="left" class="style4 style4">Orientation Date<br>
      <strong><span class="style4 style7 style8 style9">(m/d/yyyy)</span></strong></p>      </th>
    <th scope="col"><span class="style4 style4">Session</span></th>
    <th scope="col"><span class="style4 style4">XP Training Date <span class="style2"><strong><span class="style4 style7 style8"><br>
              <span class="style9">(m/d/yyyy)</span></span></strong></span></span></th>
    <th scope="col"><span class="style4 style4">Session</span></th>
    <th scope="col"><span class="style4 style4">Migration Date <span class="style2"><strong><span class="style8 style9"><span class="style4 style7 style8">(m/d/yyyy)</span></span></strong></span></span></th>
    <th scope="col"><span class="style4 style4">Migration Time</span></th>
  </tr>
  <tr>
    <td><select name="loc" class="burgundy">
	<option value="#loc#">#loc#</option>
	<option value=""></option>
	  <option value="BUFFALO">BUFFALO</option>
      <option value="EROC">EROC</option>
	  <option value="HO">HO</option>
	  <option value="NEWARK">NEWARK</option>
	  <option value="UTICA">UTICA</option>
    </select></td>
    <td><input name="flr" type="text" class="burgundy" value="#flr#" size="15" maxlength="15"></td>
    <td><input name="office" type="text" class="burgundy" value="#office#" size="15" maxlength="15"></td>
    <td><input name="ph" type="text" class="burgundy" value="#ph#" size="15" maxlength="15"></td>
    <td><input name="wrkhrs" type="text" class="burgundy" value="#wrkhrs#" size="15" maxlength="15"></td>
    <td><select name="blkberry" class="burgundy">
	<option value="#blkberry#">#blkberry#</option>
	<option value=""></option>
      <option value="Yes">Yes</option>
      <option value="No">No</option>
    </select></td>
    <td><select name="mtype" class="burgundy">
	  <option value="#mtype#">#mtype#</option>
	  <option value=""></option>
      <option value="Desktop">Desktop</option>
      <option value="Laptop">Laptop</option>
    </select></td>
    <td><select name="dispo" class="burgundy" >
	  <option value="#dispo#">#dispo#</option>
	  <option value=""></option>
	  <option value="Production">Production</option>
      <option value="Exception">Exception</option>
      <option value="Test">Test</option>
	  <option value="Decommision">Decommision</option>
	  <option value="Shared/Pooled">Shared/Pooled</option>
	  <option value="Kiosk">Kiosk</option>
	  <option value="Utility">Utility</option>
    </select></td>
    <td><input name="desc" type="text" class="burgundy" value="#itemdesc#" size="15" maxlength="25"></td>
    <td><input name="serial" type="text" class="burgundy" value="#serial#" size="15" maxlength="15"></td>
    <td><select name="model" class="burgundy">
	<option value="#replacemodel#">#replacemodel#</option>
	  <option value=""></option>
      <option value="Desktop GX110">Desktop GX110</option>
      <option value="Desktop GX150">Desktop GX150</option>
      <option value="Desktop GX240">Desktop GX240</option>
      <option value="Desktop GX260">Desktop GX260</option>
      <option value="Desktop GX270">Desktop GX270</option>
	  <option value="Desktop GX340">Desktop GX340</option>
      <option value="Laptop T23">Laptop T23</option>
      <option value="Laptop X30">Laptop X30</option>
      <option value="Laptop T30">Laptop T30</option>
      <option value="Laptop X31">Laptop X31</option>
      <option value="Laptop T40">Laptop T40</option>
    </select></td>
    <td><select name="status" class="burgundy">
	  <option value="#status#">#status#</option>
	  <option value=""></option>
      <option value="New">New</option>
      <option value="Re-Image">Re-Image</option>
    </select></td>
    <td><cfinput type="text" name="odate" value="#dateformat(odate,'m/d/yyyy')#" size="15" maxlength="15" required="no" validate="date" message="Please Enter a valid Date" class="burgundy"></td>
    <td><select name="osession" class="burgundy">
	<option value="#osession#">#osession#</option>
	<option value=""></option>
      <option value="AM">AM</option>
      <option value="PM">PM</option>
    </select></td>
    <td><cfinput type="text" name="tdate" value="#dateformat(tdate,'m/d/yyyy')#" size="15" maxlength="15" required="no" validate="date" message="Please Enter a valid Date" class="burgundy"></td>
    <td><select name="tsession" class="burgundy">
	<option value="#tsession#">#tsession#</option>
	<option value=""></option>
      <option value="AM">AM</option>
      <option value="PM">PM</option>
    </select></td>
    <td><cfinput type="text" name="actual" value="#dateformat(actualdate,'m/d/yyyy')#" size="15" maxlength="15" class="burgundy"></td>
    <td><select name="time" class="burgundy">
	<option value="#timeformat(stime,'h:mm tt')#">#timeformat(stime,'h:mm tt')#</option>
	<option value=""></option>
      <option value="9:30 AM">9:30 AM</option>
      <option value="10:00 AM">10:00 AM</option>
	  <option value="10:30 AM">10:30 AM</option>
	  <option value="11:00 AM">11:00 AM</option>
	  <option value="11:30 AM">11:30 AM</option>
	  <option value="12:00 PM">12:00 PM</option>
    </select></td>
  </tr>
</table>
</div>
<span class="style4">Comments</span><br>
<textarea name="comment" cols="85" rows="2" class="burgundy">#comment#</textarea>
<br>
<em></em><br>
<input name="submit" type="submit" id="submit" value="SUBMIT" class="burgundy">
<hr align="left" width="700" noshade>
</cfform>
</cfoutput>


	
	<!--end display-->	</td>
  </tr>
  <tr>
    <td height="18" align="left" valign="top">&nbsp;</td>
  </tr>
</table></td>
  </tr>
</table>

                <!-- InstanceEndEditable --></td>
              </tr>
            </table> </td>
        </tr>
        <tr> 
          <td valign="top" bgcolor="#FFFFFF">&nbsp;</td>
        </tr>
        <tr> 
          <td valign="top" bgcolor="#999999"><font color="#FFFFFF">This site optimally 
            viewed with Internet Explorer running on Windows 2000 or higher.</font></td>
        </tr>
      </table></td>
  </tr>
</table>
<map name="Map">
              <area shape="rect" coords="19,2,151,48" href="http://www.it.ny.frb.org/wcsd-dsi">
</map>
</body>
<!-- InstanceEnd --></html>
