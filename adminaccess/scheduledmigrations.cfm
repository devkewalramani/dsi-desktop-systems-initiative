...
<cfset edate=dateformat(now(),'m/d/yyyy')+11>
<cfelseif dateformat(now(),'dddd') eq 'Tuesday'>
<cfset bdate=dateformat(now(),'m/d/yyyy')+6>
<cfset edate=dateformat(now(),'m/d/yyyy')+10>
...
Please do not run untill friday prior to the week capturing data
<cfset bdate=dateformat(now(),'m/d/yyyy')+1>
<cfset edate=dateformat(now(),'m/d/yyyy')+5>
</cfif>
<cfoutput>
	<form name="range" method="post" action="capturedata.cfm">
  <p>
  Please enter your start date:
  <br>
  <input type=text name='bdate' size=10 maxlength=10 value="#dateformat(bdate,'m/d/yyyy')#">
  <br>
  Please enter you end date:
  <br>    
  <input type=text name='edate' size=10 maxlength=10 value="#dateformat(edate,'m/d/yyyy')#">
  <br>  
  <p>
