...
SELECT (Sum([teamoneold].[hdd])+Sum([teamoneold].[sysprep])+Sum([teamoneold].[gw])+Sum([teamoneold].[joinrb]) +Sum([teamoneold].[policy])+Sum([teamoneold].[dns])+Sum([teamoneold].[mapk]) +Sum([teamoneold].[otherapps])+Sum([teamoneold].[context])+Sum([teamoneold].[rr])) / Count(*) AS totavg, Min([teamoneold].[hdd])+Min([teamoneold].[sysprep])+Min([teamoneold].[gw])+ Min([teamoneold].[joinrb]) +Min([teamoneold].[policy])+Min([teamoneold].[dns])+Min([teamoneold].[mapk]) +Min([teamoneold].[otherapps])+Min([teamoneold].[context])+Min([teamoneold].[rr]) AS totmin, Max([teamoneold].[hdd])+Max([teamoneold].[sysprep])+Max([teamoneold].[gw])+ Max([teamoneold].[joinrb]) +Max([teamoneold].[policy])+Max([teamoneold].[dns])+Max([teamoneold].[mapk]) +Max([teamoneold].[otherapps])+Max([teamoneold].[context])+Max([teamoneold].[rr]) AS totmax
FROM teamoneold
WHERE edate Between #parsedatetime(form.bdate)# AND #parsedatetime(form.edate)# AND type='#Form.type#'
</cfquery>

<cfoutput query="avgminmax">           
<cfxml variable="avgminmax1old">
...
<cfset xml = #ToString(avgminmax1old)#>
<cffile action="write" file="#getDirectoryFromPath(getTemplatePath())#avgminmax1old.svg" output="#xml#">
<cflocation url="avgminmax1old.svg">