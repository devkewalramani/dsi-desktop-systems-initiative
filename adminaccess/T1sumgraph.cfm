...
SELECT DISTINCTROW [teamone].[tech],Avg([teamone].[hdd]) AS [hdd],Avg([teamone].[sysprep]) AS[sysprep], Avg([teamone].[joinrb]) AS [joinrb],Avg([teamone].[policy]) AS [policy],Avg([teamone].[mapk]) AS [mapk],Avg([teamone].[otherapps]) AS [otherapps],Avg([teamone].[context]) AS [context],Avg([teamone].[rr]) AS [rr]
FROM teamone
